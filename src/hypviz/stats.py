...
def _norms(coords, k):
    return L.dist(coords, L.origin(coords.shape[-1] - 1, k), k)
...
def norm_hist(coords, k=-1.0, by=None, bins=40, size=(5.2, 3.2)):
...
def depth_norm(coords, depths, k=-1.0, size=(5.2, 3.2)):
    ...
    fig = Figure(figsize=size)
    ax = _bare(fig.add_subplot())
    n, depths = _norms(coords, k), np.asarray(depths)
